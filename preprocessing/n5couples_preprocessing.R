n5couples_preprocessing <- function(df){
  # Based on nfhs_cascade/ncp_preprocessing.R
  df %>% 
  ...
  mutate(dm_disease = case_when(is.na(dm_free) ~ NA_real_,
    ...
    # NEW ---
    mutate(tobacco_any = case_when(!is.na(tobacco_none) ~ 1 - tobacco_none,
                               TRUE ~ 0)) %>% 
    mutate(glucose = case_when(is.na(glucose) | glucose > 498 ~ NA_real_,
                               TRUE ~ glucose)) %>% 
    mutate_at(vars(wealthq,swealthq_ur),function(x) factor(x,
                                                           levels=c(1:5),
                                                           labels=c("Lowest","Low","Medium","High","Highest"))) %>%
    mutate(lengthmar = case_when(age1stmarriage == 98 | is.na(age1stmarriage) ~ NA_real_,
