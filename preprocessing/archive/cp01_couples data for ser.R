...
iacr_extract <- read_dta(paste0(path_india_raw_data,"/IACR7CDT/IACR7CFL.dta"),
...
iapr_extract <- read_dta(paste0(path_india_raw_data,"/IAPR7CDT/IAPR7CFL.dta"),
...
mutate_at(vars(sb18s,sb18d,
  ...
  mutate(f_hemo_anemia = case_when(v456 >= 250 | v456 < 3 ~ NA_real_,
         ...
         f_bmi_underweight = case_when(v445 > bmi_max ~ NA_real_,
                                       v445 < bmi_cutoff[1] ~ 1,
                                       v445 >= bmi_cutoff[1] ~ 0,
                                       TRUE ~ NA_real_),
         m_bmi_underweight = case_when(hb40 > bmi_max ~ NA_real_,
                                       hb40 < bmi_cutoff[1] ~ 1,
                                       hb40 >= bmi_cutoff[1] ~ 0,
                                       TRUE ~ NA_real_),
         
         f_bmi_overweight = case_when(v445 > bmi_max ~ NA_real_,
                                      v445 >= bmi_cutoff[2] & v445 < bmi_cutoff[3] ~ 1,
                                      v445 < bmi_cutoff[2] | v445 >= bmi_cutoff[3] ~ 0,
                                      TRUE ~ NA_real_),
         m_bmi_overweight = case_when(hb40 > bmi_max ~ NA_real_,
                                      hb40 >= bmi_cutoff[2] & hb40 < bmi_cutoff[3] ~ 1,
                                      hb40 < bmi_cutoff[2] | hb40 >= bmi_cutoff[3] ~ 0,
                                      TRUE ~ NA_real_),
         
         f_bmi_obese = case_when(v445 > bmi_max ~ NA_real_,
                                 v445 >= bmi_cutoff[3] ~ 1,
                                 v445 < bmi_cutoff[3] ~ 0,
                                 TRUE ~ NA_real_),
         m_bmi_obese = case_when(hb40 > bmi_max ~ NA_real_,
                                 hb40 >= bmi_cutoff[3] ~ 1,
                                 hb40 < bmi_cutoff[3] ~ 0,
                                 TRUE ~ NA_real_),
         
         f_fasting = case_when(sb53 > 94 | sb54 > 94 ~ NA_real_,
                               sb53 > fasting_time & sb54 > fasting_time ~ 1,
                               sb53 <=fasting_time | sb54 <= fasting_time ~ 0,
                               TRUE ~ NA_real_),
         m_fasting = case_when(smb53 > 94 | smb54 > 94 ~ NA_real_,
                               smb53 > fasting_time & smb54 > fasting_time ~ 1,
                               smb53 <=fasting_time | smb54 <= fasting_time ~ 0,
                               TRUE ~ NA_real_),
         
         f_ifg = case_when(is.na(sb74) | sb74 > 498 ~ NA_real_,
                           f_self_diabetes == 1 ~ NA_real_, # Exclude those who have diabetes
                           f_fasting == 1 & sb74 >= fpgpre_cutoff & sb74 < fpg_cutoff ~ 1,
                           f_fasting == 1 & (sb74 < fpgpre_cutoff | sb74 >= fpg_cutoff) ~ 0,
         ...
         m_ifg = case_when(is.na(smb74) | smb74 > 498 ~ NA_real_,
                           ...
                           m_fasting == 1 & smb74 >= fpgpre_cutoff & smb74 < fpg_cutoff ~ 1,
                           m_fasting == 1 & (smb74 < fpgpre_cutoff | smb74 >= fpg_cutoff) ~ 0,
                           m_fasting == 0 ~ NA_real_,
                           TRUE ~ NA_real_),
         
         f_igt = case_when(is.na(sb74) | sb74 > 498 ~ NA_real_,
                           f_self_diabetes == 1 ~ NA_real_, # Exclude those who have diabetes
                           f_fasting == 0 & sb74 >= rpgpre_cutoff & sb74 < rpg_cutoff ~ 1,
                           f_fasting == 0 & (sb74 < rpgpre_cutoff | sb74 >= rpg_cutoff) ~ 0,
                           f_fasting == 1 ~ NA_real_,
                           TRUE ~ NA_real_),
         m_igt = case_when(is.na(smb74) | smb74 > 498 ~ NA_real_,
                           m_self_diabetes == 1 ~ NA_real_, # Exclude those who have diabetes
                           m_fasting == 0 & smb74 >= rpgpre_cutoff & smb74 < rpg_cutoff ~ 1,
                           m_fasting == 0 & (smb74 < rpgpre_cutoff | smb74 >= rpg_cutoff) ~ 0,
                           m_fasting == 1 ~ NA_real_,
                           TRUE ~ NA_real_),
         
         f_diagifg = case_when(is.na(sb74) | sb74 > 498 ~ NA_real_,
                               f_self_diabetes == 1 & f_fasting == 1 & sb74 >= fpgpre_cutoff & sb74 < fpg_cutoff ~ 1,
                               f_self_diabetes == 1 & f_fasting == 1 & (sb74 < fpgpre_cutoff | sb74 >= fpg_cutoff) ~ 0,
                               f_self_diabetes == 1 & f_fasting == 0 ~ NA_real_,
                               TRUE ~ NA_real_),
         m_diagifg = case_when(is.na(smb74) | smb74 > 498 ~ NA_real_,
                               m_self_diabetes == 1 & m_fasting == 1 & smb74 >= fpgpre_cutoff & smb74 < fpg_cutoff ~ 1,
                               m_self_diabetes == 1 & m_fasting == 1 & (smb74 < fpgpre_cutoff | smb74 >= fpg_cutoff) ~ 0,
                               m_self_diabetes == 1 & m_fasting == 0 ~ NA_real_,
                               TRUE ~ NA_real_),
         
         f_diagigt = case_when(is.na(sb74) | sb74 > 498 ~ NA_real_,
                               f_self_diabetes == 1 & f_fasting == 0 & sb74 >= rpgpre_cutoff & sb74 < rpg_cutoff ~ 1,
                               f_self_diabetes == 1 & f_fasting == 0 & (sb74 < rpgpre_cutoff | sb74 >= rpg_cutoff) ~ 0,
                               f_self_diabetes == 1 & f_fasting == 1 ~ NA_real_,
                               TRUE ~ NA_real_),
         m_diagigt = case_when(is.na(smb74) | smb74 > 498 ~ NA_real_,
                               m_self_diabetes == 1 & m_fasting == 0 & smb74 >= rpgpre_cutoff & smb74 < rpg_cutoff ~ 1,
                               m_self_diabetes == 1 & m_fasting == 0 & (smb74 < rpgpre_cutoff | smb74 >= rpg_cutoff) ~ 0,
                               m_self_diabetes == 1 & m_fasting == 1 ~ NA_real_,
                               TRUE ~ NA_real_),
         
         f_dm = case_when(f_self_diabetes == 1 ~ 1,
                          is.na(sb74) | sb74 > 498 ~ NA_real_,
                          f_fasting == 1 & sb74 >= fpg_cutoff ~ 1,
                          f_fasting == 0 & sb74 >= rpg_cutoff ~ 1,
                          is.na(f_fasting) & sb74 >= rpg_cutoff ~ 1,
                          f_fasting == 1 & sb74 < fpg_cutoff ~ 0,
                          f_fasting == 0 & sb74 < rpg_cutoff ~ 0,
                          is.na(f_fasting) & sb74 < rpg_cutoff ~ 0,
                          TRUE  ~ NA_real_
         ),
         m_dm = case_when(m_self_diabetes == 1 ~ 1,
                          is.na(smb74) | smb74 > 498 ~ NA_real_,
                          m_fasting == 1 & smb74 >= fpg_cutoff ~ 1,
                          m_fasting == 0 & smb74 >= rpg_cutoff ~ 1,
                          is.na(m_fasting) & smb74 >= rpg_cutoff ~ 1,
                          m_fasting == 1 & smb74 < fpg_cutoff ~ 0,
                          m_fasting == 0 & smb74 < rpg_cutoff ~ 0,
                          is.na(m_fasting) & smb74 < rpg_cutoff ~ 0,
                          TRUE  ~ NA_real_
         ),
         
         f_diagdm = case_when(
           is.na(sb74) | sb74 > 498 ~ NA_real_,
           f_self_diabetes == 1 & f_fasting == 1 & sb74 >= fpg_cutoff ~ 1,
           f_self_diabetes == 1 & f_fasting == 0 & sb74 >= rpg_cutoff ~ 1,
           f_self_diabetes == 1 & is.na(f_fasting) & sb74 >= rpg_cutoff ~ 1,
           f_self_diabetes == 1 & f_fasting == 1 & sb74 < fpg_cutoff ~ 0,
           f_self_diabetes == 1 & f_fasting == 0 & sb74 < rpg_cutoff ~ 0,
           f_self_diabetes == 1 & is.na(f_fasting) & sb74 < rpg_cutoff ~ 0,
           TRUE  ~ NA_real_
         ),
         m_diagdm = case_when(
           is.na(smb74) | smb74 > 498 ~ NA_real_,
           m_self_diabetes == 1 & m_fasting == 1 & smb74 >= fpg_cutoff ~ 1,
           m_self_diabetes == 1 & m_fasting == 0 & smb74 >= rpg_cutoff ~ 1,
           m_self_diabetes == 1 & is.na(m_fasting) & smb74 >= rpg_cutoff ~ 1,
           m_self_diabetes == 1 & m_fasting == 1 & smb74 < fpg_cutoff ~ 0,
           m_self_diabetes == 1 & m_fasting == 0 & smb74 < rpg_cutoff ~ 0,
           m_self_diabetes == 1 & is.na(m_fasting) & smb74 < rpg_cutoff ~ 0,
           TRUE  ~ NA_real_
         ),
         ...
         f_prehtn = case_when(f_self_hypertension == 1 ~ NA_real_,
                              ...
                              f_sbp >= sbppre1_cutoff & f_sbp < sbp_cutoff ~ 1,
                              f_dbp >= dbppre1_cutoff & f_dbp < dbp_cutoff ~ 1,
                              f_sbp < sbppre1_cutoff | f_sbp >= sbp_cutoff ~ 0,
                              f_dbp < dbppre1_cutoff | f_dbp >= dbp_cutoff ~ 0,
                              TRUE ~ NA_real_),
         m_prehtn = case_when(m_self_hypertension == 1 ~ NA_real_,
                              is.na(m_sbp) | is.na(m_dbp) ~ NA_real_,
                              m_sbp >= sbppre1_cutoff & m_sbp < sbp_cutoff ~ 1,
                              m_dbp >= dbppre1_cutoff & m_dbp < dbp_cutoff ~ 1,
                              m_dbp < sbppre1_cutoff | m_dbp >= sbp_cutoff ~ 0,
                              m_dbp < dbppre1_cutoff | m_dbp >= dbp_cutoff ~ 0,
                              TRUE ~ NA_real_),
         
         f_prehtn2 = case_when(f_self_hypertension == 1 ~ NA_real_,
                               is.na(f_sbp) | is.na(f_dbp) ~ NA_real_,
                               f_sbp >= sbppre2_cutoff & f_sbp < sbp_cutoff ~ 1,
                               f_dbp >= dbppre2_cutoff & f_dbp < dbp_cutoff ~ 1,
                               f_sbp < sbppre2_cutoff | f_sbp >= sbp_cutoff ~ 0,
                               f_dbp < dbppre2_cutoff | f_dbp >= dbp_cutoff ~ 0,
                               TRUE ~ NA_real_),
         m_prehtn2 = case_when(m_self_hypertension == 1 ~ NA_real_,
                               is.na(m_sbp) | is.na(m_dbp) ~ NA_real_,
                               m_sbp >= sbppre2_cutoff & m_sbp < sbp_cutoff ~ 1,
                               m_dbp >= dbppre2_cutoff & m_dbp < dbp_cutoff ~ 1,
                               m_dbp < sbppre2_cutoff | m_dbp >= sbp_cutoff ~ 0,
                               m_dbp < dbppre2_cutoff | m_dbp >= dbp_cutoff ~ 0,
                               TRUE ~ NA_real_),
         ...
         f_diagprehtn = case_when(
           is.na(f_sbp) | is.na(f_dbp) ~ NA_real_,
           f_self_hypertension == 1 & f_sbp >= sbppre1_cutoff & f_sbp < sbp_cutoff ~ 1,
           f_self_hypertension == 1 & (f_dbp >= dbppre1_cutoff & f_dbp < dbp_cutoff) ~ 1,
           f_self_hypertension == 1 & (f_sbp < sbppre1_cutoff | f_sbp >= sbp_cutoff) ~ 0,
           f_self_hypertension == 1 & (f_dbp < dbppre1_cutoff | f_dbp >= dbp_cutoff) ~ 0,
           TRUE ~ NA_real_),
         m_diagprehtn = case_when(
           is.na(m_sbp) | is.na(m_dbp) ~ NA_real_,
           m_self_hypertension == 1 & (m_sbp >= sbppre1_cutoff & m_sbp < sbp_cutoff) ~ 1,
           m_self_hypertension == 1 & (m_dbp >= dbppre1_cutoff & m_dbp < dbp_cutoff) ~ 1,
           m_self_hypertension == 1 & (m_dbp < sbppre1_cutoff | m_dbp >= sbp_cutoff) ~ 0,
           m_self_hypertension == 1 & (m_dbp < dbppre1_cutoff | m_dbp >= dbp_cutoff) ~ 0,
           TRUE ~ NA_real_),
         
         f_diagprehtn2 = case_when(
           is.na(f_sbp) | is.na(f_dbp) ~ NA_real_,
           f_self_hypertension == 1 & f_sbp >= sbppre2_cutoff & f_sbp < sbp_cutoff ~ 1,
           f_self_hypertension == 1 & (f_dbp >= dbppre2_cutoff & f_dbp < dbp_cutoff) ~ 1,
           f_self_hypertension == 1 & (f_sbp < sbppre2_cutoff | f_sbp >= sbp_cutoff) ~ 0,
           f_self_hypertension == 1 & (f_dbp < dbppre2_cutoff | f_dbp >= dbp_cutoff) ~ 0,
           TRUE ~ NA_real_),
         m_diagprehtn2 = case_when(
           is.na(m_sbp) | is.na(m_dbp) ~ NA_real_,
           m_self_hypertension == 1 & (m_sbp >= sbppre2_cutoff & m_sbp < sbp_cutoff) ~ 1,
           m_self_hypertension == 1 & (m_dbp >= dbppre2_cutoff & m_dbp < dbp_cutoff) ~ 1,
           m_self_hypertension == 1 & (m_dbp < sbppre2_cutoff | m_dbp >= sbp_cutoff) ~ 0,
           m_self_hypertension == 1 & (m_dbp < dbppre2_cutoff | m_dbp >= dbp_cutoff) ~ 0,
           TRUE ~ NA_real_),
         ...
         f_diaghtn = case_when(
           is.na(f_sbp) | is.na(f_dbp) ~ NA_real_,
           f_self_hypertension == 1 & f_sbp >= sbp_cutoff ~ 1,
           f_self_hypertension == 1 & f_dbp >= dbp_cutoff ~ 1,
           f_self_hypertension == 1 & f_sbp < sbp_cutoff ~ 0,
           f_self_hypertension == 1 & f_dbp < dbp_cutoff ~ 0,
           TRUE ~ NA_real_),
         
         m_diaghtn = case_when(
           is.na(m_sbp) | is.na(m_dbp) ~ NA_real_,
           m_self_hypertension == 1 & m_sbp >= sbp_cutoff ~ 1,
           m_self_hypertension == 1 & m_dbp >= dbp_cutoff ~ 1,
           m_self_hypertension == 1 & m_sbp < sbp_cutoff ~ 0,
           m_self_hypertension == 1 & m_dbp < dbp_cutoff ~ 0,
           TRUE ~ NA_real_)
