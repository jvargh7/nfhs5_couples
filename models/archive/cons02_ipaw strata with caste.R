...
strata_associations <- map_dfr(strata_vars,
                               function(s){
                                 ...
                                 ipaw_dm_strata_design <- ipaw_dm_df %>% 
                                 ...
                                 ipaw_htn_strata_design <- ipaw_htn_df %>% 
                                   mutate(weight2 = weight/(1-ipaw)) %>% 
                                   dplyr::rename(strata = s) %>% 
                                   dplyr::filter(strata == 1) %>% 
                                   as_survey_design(.data = .,
...
write_csv(strata_associations,paste0(path_couples_folder,"/working/concordance/ipaw strata specific spousal status associations long.csv"))
